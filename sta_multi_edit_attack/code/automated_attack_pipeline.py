...
def compute_sta_stats(input_ids, rng_device='cuda' if torch.cuda.is_available() else 'cpu'):
    ...
    # Theoretical mean and standard deviation under H0
...
def main():
    ...
    print(
        f"Green Pair Count: "
        f"{orig_stats['green_count']} / {orig_stats['pair_count']}"
    )
    ...
    print(
        f"Green Pair Count: "
        f"{attacked_stats['green_count']} / "
        f"{attacked_stats['pair_count']}"
    )
    print(
        f"Delta Z: "
        f"{orig_stats['z'] - attacked_stats['z']:.4f}"
    )
    ...
    print(
        f"[✓ SAVED] Module 1 result written to: "
        f"{OUTPUT_PATH}\n"
    )
